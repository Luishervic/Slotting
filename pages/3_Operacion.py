"""Paso 3 — Simulación operativa de pickeo y recorridos.
...
if st.session_state.get("slots"):
    ...
    slots_validos = [s for s in st.session_state["slots"]
                     if rectangulo_en_poligono(s, cfg_sf.perimetro)
                     and S.rectangulo_en_zonas(s, cfg_sf.zonas)]
    res_aco = S.distribuir(df, slots_validos, cfg_sf,
...
meta_escenario = st.session_state.get("meta_escenario", {})
st.caption("Simulando sobre el **layout actual** (página 🏗️ Layout)"
           + (f" · escenario: **{meta_escenario.get('nombre')}**"
              if meta_escenario else ""))
...
st.session_state["ultima_simulacion"] = {
    "firma": {
        "slots_rev": int(st.session_state.get("slots_rev", 0)),
        "nombre": meta_escenario.get("nombre"),
        "columna": meta_escenario.get("columna"),
        "factor": float(meta_escenario.get("factor", 1)),
    },
    "kpis": dict(k),
    "demanda": "csv" if pedidos_reales is not None else "sintetica",
    "config": dict(cfg_sim.__dict__),
}
...
st.divider()
with st.expander("Guardar resultado operativo", expanded=False):
    scenario_name = st.text_input(
        "Nombre de la simulación",
        value="Simulación operativa",
    ...
    if st.button(
        ...
        def _save_operation_scenario():
            ...
            scenario_id = store.save(
                ...
                kpis=dict(k),
                artifacts={
                    "pedidos_simulados": out["pedidos"],
                    "visitas": out["visitas"],
                    "ubicaciones": pd.DataFrame(
                        res_aco.get("slots", [])
                    ),
                    "asignaciones": res_aco["asignaciones"],
                },
            )
            st.session_state["ultimo_escenario_id"] = scenario_id

        confirmar_accion(
            titulo="Guardar simulación",
            detalle=(
